...
db_job_type = DB_JOB_TYPE.INSERT_PRODUCT_FORCE.name

if main_category is not None:
	proxy_manager.changeProxy()

	sub_category_list = naverShopping.get_sub_category_list(proxy_manager, main_category)
	...
	cmd = f"python.exe main.py {db_job_type} {config.CFG_PROXY_FILENAME}"

	if os.path.exists('main.exe'):
		cmd = f"main.exe {db_job_type} {config.CFG_PROXY_FILENAME}"
	...
	for main_category in config.CFG_NAVER_SHOPPING_CATEGORY_CODE_LIST:
		...
		commonLib.print_log(LOGLEVEL.D, "main_category Loop Begin : " + main_category)
		sub_category_list = naverShopping.get_sub_category_list(proxy_manager, main_category)
		commonLib.print_log(LOGLEVEL.D, "main_category Loop End : " + main_category)

		for sub_category in sub_category_list:
			cmd = f"python.exe main.py {sub_category} {db_job_type} %s" % config.CFG_PROXY_FILENAME

			if os.path.exists('main.exe'):
				cmd = f"main.exe {sub_category} {db_job_type} %s" % config.CFG_PROXY_FILENAME
